...
class Enemy:
    ...
    def move_towards_player(self, player_x, player_y):
        dx = player_x - self.__x
        dy = player_y - self.__y
        distance = max(abs(dx), abs(dy))
        if distance != 0:
            step_x = dx / distance * self.__speed
            step_y = dy / distance * self.__speed
            if self.check_oob(self.__x + step_x, self.__y + step_y):
                self.__x += step_x
                self.__y += step_y
